...
public class VoiceSpritzSuperEqualizer : VoiceSpritzEffect
{
    float db2log;
    float pi;
    float halfpi;
    ...
    public VoiceSpritzSuperEqualizer(float lowFrequenciesDrivePercent = 0, float lowFrequenciesGainDb = 0, float middleFrequenciesDrivePercent = 0, float middleFrequenciesGainDb = 0, float highFrequenciesDrivePercent = 0, float highFrequenciesGainDb = 0, float lowMiddleFrequencyHz = 240, float middleHighFrequencyHz = 2400)
    {
        ...
        // Low-middle frequency (Hz) => Default (240), Minimum (60), Maximum (500)
        // Middle-high frequency (Hz) => Default (2400), Minimum (510), Maximum (10000)
    ...
    public override void Process(float[] theBuffer, int samplesRead)
    {
        for (int j = 0; j < samplesRead; j++)
        {
            ...
            float wet0_l = mixlgd * low_l * (1 - Math.Abs(low_l * drivel2));
            float wet0_m = mixmgd * mid_l * (1 - Math.Abs(mid_l * drivem2));
            float wet0_h = mixhgd * high_l * (1 - Math.Abs(high_l * driveh2));
            ...
            float wet1_l = mixlgd * low_r * (1 - Math.Abs(low_r * drivel2));
            float wet1_m = mixmgd * mid_r * (1 - Math.Abs(mid_r * drivem2));
            float wet1_h = mixhgd * high_r * (1 - Math.Abs(high_r * driveh2));
